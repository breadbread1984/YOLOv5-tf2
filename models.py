...
def YOLOv5(input_shape = (608, 608, 3), class_num = 80, anchor_num = 3):
  ...
  large_feature, results = ResBlock(results.shape[1:], filters = 512, blocks = 1, output_filters = 256, output_kernel = (1, 1))(results); # large_feature.shape = (batch, h / 8, w / 8, 256), results.shape = (batch, h / 16, w / 16, 256)
  ...
  middle_feature, results = ResBlock(results.shape[1:], filters = 256, blocks = 1, output_filters = 128, output_kernel = (1, 1))(results); # middle_feature.shape = (batch, h / 4, w / 4,128), results.shape = (batch, h / 8, w / 8, 128)
  ...
  small_feature, results = ResBlock(results.shape[1:], filters = 128, blocks = 1, output_filters = 128, output_kernel = (1, 1), downsample = True)(results); # small_feature.shape = (batch, h / 4, w / 4, 128), results.shape = (batch, h / 8, w / 8, 128)
