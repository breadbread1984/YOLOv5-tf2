...
def ResBlock(input_shape, filters, blocks):
  ...
  results = ConvBlockMish(results.shape[1:], 2 * filters, (3, 3))(results);
  return tf.keras.Model(inputs = inputs, outputs = results);
...
def Body(input_shape):
  ...
  rb1 = ResBlock(cb.shape[1:], filters = 64, blocks = 1)(cb); # rb1.shape = (batch, h, w, 128)
  rb2 = ResBlock(rb1.shape[1:], filters = 128, blocks = 3)(rb1); # rb2.shape = (batch, h, w, 256)
  rb3 = ResBlock(rb2.shape[1:], filters = 256, blocks = 3)(rb2); # rb3.shape = (batch, h, w, 512)
